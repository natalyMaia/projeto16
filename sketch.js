...
var scoreR = 0,scoreB = 0,scoreG = 0,scoreP = 0,Hit = 0;
function preload(){
...
function setup() {
  ...
  barra = createSprite(400,200,5,400);
  barra.shapecolor = "lighblue";
...
function draw() {
  ...
  if(gameState === PLAY){
    ...
    if (barra.isTouching(redB)) {
      Hit = Hit + 1;
      //Use função destroyEach() para destruir o grupo redBalloon (balão vermelho)
      redB.destroyEach();
      if (Hit == 5){
        gameState = END
      }
    ...
    barra.bounceOff(redB);
    barra.bounceOff(pinkB); 
    barra.bounceOff(blueB); 
    barra.bounceOff(pinkB); 
  ...
  drawSprites();
  text("Red: " + scoreR, 300,40);
  text("Blue: " + scoreB, 300,50);
  text("Green: " + scoreG, 300,60);
  text("Pink: " + scoreP, 300,70);
